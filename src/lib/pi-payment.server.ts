...
async function piRequest(apiKey: string, path: string, method = "GET", body?: object) {
...
export async function approvePayment(input: { escrowId: string; paymentId: string }, context: AuthContext, apiKey: string) {
  ...
  const payment = await piRequest(apiKey, encodeURIComponent(input.paymentId));
  validatePayment(payment, escrow, input.paymentId);
  if (!payment.status?.developer_approved) await piRequest(apiKey, `${encodeURIComponent(input.paymentId)}/approve`, "POST");
  return { ok: true };
}

export async function completePayment(input: { escrowId: string; paymentId: string; txId: string }, context: AuthContext, apiKey: string) {
  const escrow = await ownedEscrow(context, input.escrowId);
  if (escrow.status === "funded" && escrow.pi_payment_id === input.paymentId) return { ok: true, txId: input.txId };
  if (escrow.status !== "awaiting_payment") throw new Error("This escrow is not awaiting payment");
  let payment = await piRequest(apiKey, encodeURIComponent(input.paymentId));
  validatePayment(payment, escrow, input.paymentId);
  if (!payment.status?.developer_approved) throw new Error("Pi payment has not been approved");
  if (!payment.status?.developer_completed) payment = await piRequest(apiKey, `${encodeURIComponent(input.paymentId)}/complete`, "POST", { txid: input.txId });
...
export async function recoverPayment(input: { paymentId: string; escrowId: string; txId?: string }, context: AuthContext, apiKey: string) {
  const escrow = await ownedEscrow(context, input.escrowId);
  const payment = await piRequest(apiKey, encodeURIComponent(input.paymentId));
  validatePayment(payment, escrow, input.paymentId);
  ...
  await completePayment({ escrowId: input.escrowId, paymentId: input.paymentId, txId }, context, apiKey);
